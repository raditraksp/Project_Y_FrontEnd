...
export default function ManageOrderAdmin() {
    ...
    const onRejected = (orders_id) => {   
        const message = msgAdmin.current.value
        const data = {message}

        const config = {headers: {Authorization: token}}
        axios.post(`/orders/${orders_id}/rejected/admin/`, data, config)
        .then((res) => { 
            getData() 
        })
    }
                        ...
                        <tr> 
                            ...
                            <td>
                                ...
                                <div className="container_fluid">
                                    <div className="row">
                                        ...
                                        <div className="col-6">
                                            <button type="button" onClick={toggle2} className="btn btn-danger mb-2 px-4 btn-block">Reject</button>
                                        </div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <Modal isOpen={modal2} toggle={toggle2}>
                                <div>
                                    ...
                                    <textarea ref={msgAdmin} type='text' defaultValue={orders.message_admin} className='form-control' required/>
                                </div>
                                <div className="container_fluid">
                                    <div className="row">
                                        <div className="col-6">
                                            <button type="button" onClick={() => {onRejected(order.id)}} className="btn btn-primary btn-block">Reject</button>
                                        </div>
                                        <div className="col-6">
                                            <button type="button" onClick={toggle2} className="btn btn-danger mb-2 px-4 btn-block">Cancel</button>
                                        </div>
                                    </div>
                                </div>
                            </Modal>
